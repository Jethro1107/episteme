...
export class ZoteroClient {
    ...
    async fetchItems(onProgress) {
        ...
        try {
            ...
            const countResponse = await this.fetchWithRetry(() => this.api.items().top().get({ limit: 0 }), timeout, maxRetries);
            const multiResponse = countResponse;
            ...
            while (start < totalResults) {
                const response = await this.fetchWithRetry(() => this.api.items().top().get({
                    limit,
                    start,
                    sort,
                    direction
     ...
     */
    async fetchWithRetry(fn, timeout, maxRetries) {
        let lastError = null;
...
// Factory
// ----------------------------------------------------------------------------
export function createZoteroClient(config, cacheDir, ttlHours) {
...
// ----------------------------------------------------------------------------
export function validateZoteroConfig(config) {
    if (!config.apiKey)
...
//# sourceMappingURL=zotero-client.js.map