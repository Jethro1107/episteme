 ...
 */
export async function getAttachments(itemKey, config) {
    const ApiClient = await getZoteroApiClient();
    const api = ApiClient(config.apiKey).library(config.libraryType === 'group' ? 'group' : 'user', parseInt(config.libraryId, 10));
    try {
 ...
 */
export async function findPdfAttachment(itemKey, config) {
 ...
 */
export async function downloadAttachment(attachmentKey, config, tempDir) {
    const ApiClient = await getZoteroApiClient();
    const api = ApiClient(config.apiKey).library(config.libraryType === 'group' ? 'group' : 'user', parseInt(config.libraryId, 10));
    try {
        // Fetch attachment metadata to get the download URL
        const response = await api.items(attachmentKey).get();
        const attachment = response.getData();
        const links = response.getLinks();
 ...
 */
export async function convertPdfToMarkdown(pdfPath, outputPath, timeoutMs = 120000) {
 ...
 */
export async function downloadAndConvertPdf(itemKey, config, tempDir) {
    // Find PDF attachment
    ...
    try {
        ...
        return { content: null, pdfPath: null, error: error.message };
    }
}
//# sourceMappingURL=zotero-downloader.js.map